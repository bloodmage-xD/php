<?php
  ...
  function validate_state($state, $errors=array()) {
    ...
    else if (!has_length($state['name'] , array('min' => 2, 'max' => 50)))
      $errors[] = "State length must be between 2 and 50 chars";
    else if (preg_match("/[^a-zA-Z]/" , $state["name"]))
      $errors[] = "State must have a valid name";

    if (is_blank($state['code']))
      $errors[] = "State must have a a code";
    else if (!has_length($state['code'], array('min' => 2, 'max' => 12)))
      $errors[] = "State code must be between 2 and 12 chars";
    else if (preg_match("/[^a-zA-Z]/" , $state["code"]))
      $errors[] = "State must have a valid code";

    if (is_blank($state['country_id']))
      $errors[] = "State's country_id cannot be blank";
    else if (!has_length($state['country_id'], array('min' => 2, 'max' => 12)))
      $errors[] = "State's country id must be between 2 and 12 chars";
      else if (preg_match("/[^a-zA-Z]/" , $state["country_id"]))
      $errors[] = "Country id must be between 2 and 12 chars";
    return $errors;
  }
  ...
  function validate_salesperson($salesperson, $errors=array()) {
    // TODO add validations
      ...
      else if (!has_length($salesperson['first_name'] , array('min' => 2, 'max' => 100 )))
        $errors[] = "firstname must be between 2 and 100 chars";
      else if (!has_valid_name($salesperson['first_name']))
        $errors[] = "first name is invalid";

      if (is_blank($salespersonp['last_name']))
        $errors[] = "last name cannot be blank";
      else if (!has_length($salesperson['last_name'] , array('min' => 2, 'max' => 100)))
        $errors[] = "last name must be between 2 and 100 chars";
      else if (!has_valid_name($salesperson['last_name']))
        $errors[] = "last name is invalid";

      if (is_blank($salesperson['phone']))
        $errors[] = "Phone cannot be blank";
      else if (!has_length($salesperson['phone'] , array('min' =>10 ,'max' =>  15)))
        $errors[] = "phone must be between 10 and 15 chars";
      else if (!has_valid_phone_number($salesperson['phone']))
        $errors[] = "phone number must be valid";

      if (is_blank($salesperson['email']))
        $errors[] = "email cannot be blank";
      else if (!has_length($salesperson['email'] , array('min' => 3, 'max' => 50)))
        $errors[] = "email must be between 3 and 50 chars";
  ...
  function insert_salesperson($salesperson) {
    ...
    $sql = "INSERT INTO SALESPERSON"; // TODO add SQL
  ...
  function update_salesperson($salesperson) {
    ...
    $sql = "UPDATE salesperson SET"; // TODO add SQL
